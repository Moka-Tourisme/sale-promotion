...
class PoSOrder(models.Model):
    ...
    def _compute_loyalty_card_count(self):
        ...
        """
        for order in self:
            cards = self.env['loyalty.card'].search([('source_pos_order_id', '=', order.id)])
            order.loyalty_card_count = len(cards)

    def action_open_pos_order_loyalty_cards(self):
        """
        ...
        action.update({
            ...
            'context': {'default_source_pos_order_id': self.id},
        })
        return action
