...
class Giftcard(models.Model):
    ...
    def _cron_gift_card_expired(self):
        gift_card_expired_with_balance_count = self.env['gift.card'].search_count([
            ('state', '=', 'expired'), ('balance', '>', 0), ('used_by_cron', '=', False)
        ])
        print("gift_card_expired_with_balance_count", gift_card_expired_with_balance_count)
        gift_card_expired_with_balance = self.env['gift.card'].search([
            ('state', '=', 'expired'), ('balance', '>', 0), ('used_by_cron', '=', False)
        ])
        gift_card_to_expire_count = self.env['gift.card'].search_count([
            ('state', '=', 'valid'), ('expired_date', '=', fields.Date.today()), ('balance', '>', 0),
            ('used_by_cron', '=', False)
        ])
        print("gift_card_to_expire_count", gift_card_to_expire_count)
        ...
        for gift_card in gift_cards:
            if gift_card.balance > 0:
                ...
                if accounts:
                    ...
                    if account_move:
                        print("GIFT CARD BALANCE", gift_card.balance)
                        account_move.write({
                            'line_ids': [
                                (0, 0, {
                                    'name': gift_card.code,
