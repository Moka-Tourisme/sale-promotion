...
class SaleOrder(models.Model):
    ...
    def _pay_with_gift_card(self, gift_card):
        ...
        elif gift_card.limited_to_product_id and gift_card.limited_to_product_id not in self.order_line.mapped(
                'product_id.product_tmpl_id'):
            error = _('Gift Card restricted to specific product.')
        elif gift_card.limited_to_category_id and gift_card.limited_to_category_id not in self.order_line.mapped(
                'product_id.product_tmpl_id').mapped('categ_id'):
            error = _('Gift Card restricted to specific category.')
        elif gift_card.limited_to_product_list and self.order_line.mapped(
                'product_id.product_tmpl_id') not in gift_card.limited_to_product_list:
            error = _('Gift Card restricted to specific product list.')
        ...
        if not error and amount > 0:
            ...
            if gift_card.limited_to_product_id and not self.order_line.filtered(
                    lambda line: line.product_id.product_tmpl_id == gift_card.limited_to_product_id):
                gift_card.unlink()
                return error
            if gift_card.limited_to_category_id and not self.order_line.filtered(
                    lambda line: line.product_id.product_tmpl_id.categ_id == gift_card.limited_to_category_id):
                gift_card.unlink()
                return error
            if gift_card.limited_to_product_list and not self.order_line.filtered(
                    lambda line: line.product_id.product_tmpl_id in gift_card.limited_to_product_list):
                gift_card.unlink()
                return error
    ...
    def _recompute_gift_card_lines(self):
        for record in self:
            ...
            for gift_card_line in gift_payment_lines.sorted(lambda line: line.gift_card_id.expired_date):
                if gift_payment_lines.filtered(
                        lambda line: line.gift_card_id.limited_to_product_id in record.order_line.mapped(
                            'product_id.product_tmpl_id')):
                    continue
                if gift_payment_lines.filtered(
                        lambda line: line.gift_card_id.limited_to_category_id in record.order_line.mapped(
                            'product_id.product_tmpl_id').mapped('categ_id')):
                    continue
                if gift_payment_lines.filtered(
                        lambda line: any(product in line.gift_card_id.limited_to_product_list for product in
                                         record.order_line.mapped('product_id.product_tmpl_id'))):
                    continue
                else:
